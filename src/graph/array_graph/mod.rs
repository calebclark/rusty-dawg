...
use crate::memory_backing::{ArrayMemoryBacking, CacheConfig, DiskVec, InternallyImmutableVecBacking};
    ...
    N: Weight + Clone,
{
    pub fn new(mutable_graph: AvlGraph<N, E, Ix>) -> Self {
        ...
        Self::new_mb(mutable_graph, mb, CacheConfig { node_cache_size: 0, edge_cache_size: 0 })
    }
...
where
    ...
    Ix: IndexType,
{
    pub fn new_mb(mutable_graph: AvlGraph<N, E, Ix>, mb: Mb, cache_config: CacheConfig) -> Self {
        /* TODO make an online way to do this that does not require double the RAM if copying from
           ram AVLGraph to a ram array graph. */

        let mut nodes = mb.new_array_node_vec(Some(mutable_graph.node_count()),
                                    cache_config.node_cache_size);
        let mut edges = mb.new_array_edge_vec(Some(mutable_graph.edge_count()),
        ...
        while node_index < mutable_graph.node_count() {
            // retrieve the node to copy
            let to_copy = mutable_graph.get_node(NodeIndex::new(node_index));

            // default values
            let mut first_edge : EdgeIndex<Ix> = EdgeIndex::end();
            let mut num_edges = 0;

            if to_copy.get_first_edge() != EdgeIndex::end() {
                first_edge = EdgeIndex::new(edge_index);
                for avl_edge in mutable_graph.ordered_edges(NodeIndex::new(node_index)) {
                    num_edges += 1;
                    edges.push(ArrayEdge { weight: avl_edge.get_weight(), target: avl_edge.get_target()});
                    edge_index += 1;
                }
            }
            nodes.push(ArrayNode {
                weight: to_copy.get_weight(),
                first_edge,
                num_edges,
            });
            node_index += 1;
    ...
    */
    fn binary_search(
        ...
        match cmp.compare(&target_weight, &mid_weight) {
            ...
            Ordering::Less => {
                self.binary_search(start, mid, target_weight, cmp)
            }
            Ordering::Greater =>
                self.binary_search(mid + 1, stop, target_weight, cmp),
        }
    }
    ...
    Ix: IndexType,
{
    pub fn new(graph: &'a ArrayGraph<N, E, Ix, Mb>, node: NodeIndex<Ix>) -> Self {
        ...
        let end = index + graph.nodes.index(node.index()).get_num_edges();
