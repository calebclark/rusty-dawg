...
use crate::memory_backing::{ArrayMemoryBacking, CacheConfig, DiskVec, ImmutableVecBacking};
    ...
    Ix: IndexType,
{
    pub fn new_mb(mutable_graph: AvlGraph<N, E, Ix>, mb: Mb, cache_config: CacheConfig) -> Self {
        ...
        let nodes = mb.new_array_node_vec(Some(mutable_graph.node_count()),
                                    cache_config.node_cache_size);
        let edges = mb.new_array_edge_vec(Some(mutable_graph.edge_count()),
    ...
    Ix: IndexType,
{
    ...
    pub fn get_edge_by_weight_cmp(
        ...
        let num_edges = self.get_node(a).get_num_edges();
        if num_edges == 0 {
            return None;
        }
        let first_edge = self.get_node(a).get_first_edge();
        self.binary_search(first_edge, first_edge + num_edges, weight, cmp)
    }
    ...
    Mb::ArrayEdgeRef: Sized,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if (self.index == self.end) {
            ...
            self.index += 1;
            Some(self.graph.edges.index(to_return).index())
        }
    }
}
...
{
    pub fn new(graph: &'a ArrayGraph<N, E, Ix, Mb>, node: NodeIndex<Ix>) -> Self {
        let index = graph.nodes.index(node.index()).get_first_edge();
