...
use crate::memory_backing::{ArrayMemoryBacking, CacheConfig, DiskBacking, RamBacking};
...
where
    Ix: IndexType,
    W: Weight + Clone,
    Mb: ArrayMemoryBacking<W, CdawgEdgeWeight<Ix>, Ix>,
{
    tokens: Rc<RefCell<dyn TokenBacking<u16>>>,
    graph: ArrayGraph<W, CdawgEdgeWeight<Ix>, Ix, Mb>,
    source: NodeIndex<Ix>,
    ...
    W: Weight + Serialize + for<'de> Deserialize<'de> + Clone,
{
    pub fn new(mutable_cdawg : Cdawg<W, Ix>) -> Self {
        ...
        Self::new_mb(mutable_cdawg, mb, CacheConfig {edge_cache_size: 0, node_cache_size: 0})
    }
}
    ...
    Mb::ArrayEdgeRef: Copy,
{
    pub fn new_mb(mutable_cdawg : Cdawg<W, Ix>, mb: Mb, cache_config: CacheConfig) -> ImmutableCdawg<W, Ix, Mb> {
        let (tokens, old_graph, source_index, _, end_position)
            = mutable_cdawg.get_data_ownership();
        let graph: ArrayGraph<W, CdawgEdgeWeight<Ix>, Ix, Mb>
